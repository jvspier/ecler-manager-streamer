...
class Poller:
    ...
    # --- batch -----------------------------------------------------------
    #: Receivers switched at once. One, so a batch does exactly what switching
    #: them by hand does. (It was four. Receivers moved onto the software
    #: stream then reported Unlock, but so did ones moved one at a time and
    #: ones switched by hand -- the flag drops by itself on that stream -- so
    #: concurrency was not the cause. One is simply the cautious choice.)
    BATCH_CONCURRENCY = 1
